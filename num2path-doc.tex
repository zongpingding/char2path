...
\let\orithelstnumber\thelstnumber
  \def\thelstnumber{%
    \numtopath[path fading = east, fill = blue]
      {\orithelstnumber}
  }
\begin{verbatim}
  \let\orithelstnumber\thelstnumber
...
\subsection{Use in text\texorpdfstring{\color{red}[NEEDS TO BE FIXED!]}{}}

Commands for changing font's size like \cs[no-index]{small},
\cs[no-index]{Large} can be directly applied to \cs{numtopath}.
...
\begin{center}
  \begin{tikzpicture}[every node/.style = {font = \ttfamily}]
    \draw [ -stealth, gray!25, line width = 1em ] (0,0) --++ (.96\linewidth,0);
    \node [ above = 1ex ] at (.06\linewidth,0) {\small 0123456789};
    \node [ above = 1ex ] at (.30\linewidth,0) {\normalsize 0123456789};
    \node [ above = 1ex ] at (.54\linewidth,0) {\large 0123456789};
    \node [ above = 1ex ] at (.78\linewidth,0) {\huge 0123456789};
    \node [ below = 1ex ] at (.06\linewidth,0)
      {\small     \numtopath[fill = orange]{0123456789}};
    \node [ below  = 1ex] at (.30\linewidth,0)
      {\normalsize\numtopath[fill = green]{0123456789}};
    \node [ below = 1ex ] at (.54\linewidth,0)
      {\large     \numtopath[fill = blue]{0123456789}};
    \node [ below = 1ex ] at (.78\linewidth,0)
      {\huge      \numtopath[fill = purple]{0123456789}};
    \makeatletter
    \node at (.06\linewidth,0) {\small\f@size pt};
    \node at (.30\linewidth,0) {\normalsize\f@size pt};
    \node at (.54\linewidth,0) {\large\f@size pt};
    \node at (.78\linewidth,0) {\huge\f@size pt};
  \end{tikzpicture}
\end{center}
