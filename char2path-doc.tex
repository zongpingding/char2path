...
\fbox{Under Construction, waiting to \texttt{@Eureka}'s \textcolor{red}{DATA}}

\begin{example}
 \chartopath[path fading = north]{ABCDEFGHIJKLMNOPQRSTUVWXYZ}
\end{example}

\begin{example}
 \chartopath[path fading = south]{abcdefghijklmnopqrstuvwxyz}
\end{example}
...
\begin{itemize}
  \item Merge the ``Fading string'' module (commmand) into \cs{chartopath}.
  \item Function: ``Use in text'' needs to optimize: reduce the deviation.
  ...
  \end{verbatim}
  \item Patterns could not accept values including ``space'', for example
  \begin{verbatim}
