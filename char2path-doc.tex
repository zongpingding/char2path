...
\begin{function}[updated=2025-07-26]{\chartopath}
  ...
  to path.
  \begin{keyval}
    \item [\key{rescan}] \val{bool} Rescan tokens \verb|$|, \verb|&|, and \verb|~| with catcode 12 (other). 
    (Default: |true|).
...
\begin{function}[added=2025-07-26]{\ctplogoimage, \ctplogofont}
  \begin{syntax}
    \cs{ctplogoimage}\marg{image}
    \cs{ctplogofont}\marg{font}
  \end{syntax}
  Customize the font and background image used by \cs{ctpTeX} and \cs{ctpLaTeX}.
\end{function}
...
\begin{example}
\charpathclip[font=lmr, scale=2.5, 
              offset={(1, 2cm)}]{H}
\end{example}
