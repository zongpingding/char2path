...
\title{^^A
  The \pkg{char2path} Package\thanks{%
...
\begin{center}
  \begin{tikzpicture}[every node/.style = {font = \ttfamily}]
    \draw [ -stealth, gray!25, line width = 1em ] (0,0) --++ (.93\linewidth,0)
     node [ black ] {|\f@size|};
    ...
    \node at (.06\linewidth,0)
      {\small     \scalebox{\fpeval{10/\f@size}}{\small     \f@size pt}};
    \node at (.24\linewidth,0)
      {\normalsize\scalebox{\fpeval{10/\f@size}}{\normalsize\f@size pt}};
    \node at (.45\linewidth,0)
      {\large     \scalebox{\fpeval{10/\f@size}}{\large     \f@size pt}};
    \node at (.72\linewidth,0)
      {\huge      \scalebox{\fpeval{10/\f@size}}{\huge      \f@size pt}};
    \makeatother
  \end{tikzpicture}
\end{center}
...
\begin{example}
  \chartopath[path fading = east, fill = red]{1234567890}
\end{example}

\begin{example}
  \chartopath[path fading = west, fill = red]{1234567890}
\end{example}
    ...
    \cs_new_protected:Npn \__num_to_path_draw:nn #1#2
      {
        \tikz
          \node [ inner~sep = .53pt,%minimum~width = .7*\f@size (or 1ex?),
                  scale = \prop_item:Ne \c_chartopath_scale_prop 
                            { \g__chartopath_scale_tl } * \f@size/9.8
                ] {\tikz \path[#1] #2;};
      }
  \end{verbatim}
\end{itemize}
