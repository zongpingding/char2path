...
\begin{example}
 \chartopath{llm}{!\#\$\%&'()*+,-./01234567}
 \chartopath{llm}{89:;<=>?@ABCDEFGHIJKLMNO}
 \chartopath{llm}{PQRSTUVWXYZ[ ]^_`abcdefg}
 \chartopath{llm}{hijklmnopqrstuvwxyz{ }~}
\end{example}
...
\begin{example}
 \chartopath[draw = none, fill = blue]{llm}{0123456789}
\end{example}

\begin{example}
 \chartopath[draw = blue, fill = red]{llm}{0123456789}
\end{example}

\begin{example}
 \chartopath[draw = red, scale = 2]{llm}{0123456789}
\end{example}
...
\begin{example}
  \chartopath[draw = none, path fading = south,
              scale = 2, fill = teal] {llm} {0123456789}
\end{example}
\begin{texnote}
...
\begin{example}
  \chartopath[draw = black, pattern = crosshatch,
              scale = 2] {llm} {0123456789}
\end{example}
...
\begin{center}
  \begin{tikzpicture}[every node/.style = {font = \ttfamily}]
    ...
    \node [ below = 1ex ] at (.06\linewidth,0)
      {\small     \chartopath[fill = orange]{llm}{0123456789}};
    \node [ below  = 1ex] at (.24\linewidth,0)
      {\normalsize\chartopath[fill = green]{llm}{0123456789}};
    \node [ below = 1ex ] at (.45\linewidth,0)
      {\large     \chartopath[fill = blue]{llm}{0123456789}};
    \node [ below = 1ex ] at (.72\linewidth,0)
      {\huge      \chartopath[fill = purple]{llm}{0123456789}};
    \makeatletter
...
\begin{itemize}
  \item [\textcolor{red}{\textsf{Enhanced, Bug}}]
  Patterns could not accept values including ``space'', for example
  \begin{verbatim}
    \chartopath[draw = black, pattern = north east lines,
                scale = 2] {llm} {0123456789}
  \end{verbatim}
  will return error(s): |Package pgf: Undefined pattern `north east lines'.|
  \textcolor{red}{IT SEEMS THAT this is caused by the expansion of arguments.}
  \item [\textcolor{red}{\textsf{Enhanced}}] Baselines of
  Alpha-small \& Alpha-caps need to optimize.
  \item [\textcolor{green}{\textsf{Function}}] Merge the ``Fading string'' module (commmand) into \cs{chartopath}.
  \item [\textcolor{blue}{\textsf{Detail}}] Function: ``Use in text'' needs to optimize: reduce the deviation.
  Is |.53pt|, |\f@size/9.8| ... the best practice? Or should we use |minimum width|?
  \begin{verbatim}
    \cs_new_protected:Npn \@@_draw:nn #1#2
      {
        \tikz
          \node [ inner~sep = .53pt,%minimum~width = .7*\f@size (or 1ex?),
                  scale = \prop_item:Ne \c_@@_scale_prop 
                            { \g_@@_scale_tl } * \f@size/9.8
                ] {\tikz \path[#1] #2;};
      }
  \end{verbatim}
\end{itemize}
