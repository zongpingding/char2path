...
def coor_transform(
    ...
    return coor_pattern.sub(replacer, data)

# def transform(x, y):
#     return f"({x + 10:.2f}, {y:.2f})"

# new_data = coor_transform(data, lambda x, y: f"({x + 20:.3f}, {y:.3f})")
new_data = coor_transform(data, lambda x, y: f"({x + 20:.3f}+\\ctpXshift, {y:.3f}+\\ctpYshift)")
print(new_data)